...
function CertCard({ c, i, onOpen }: { c: typeof certs[0]; i: number; onOpen: () => void }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative w-full aspect-[4/3] overflow-hidden">
        ...
        {/* Tag badge */}
        <div className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] font-mono-code font-medium"
          style={{ background: `${c.accent}20`, color: c.accent, border: `1px solid ${c.accent}35`, backdropFilter: "blur(8px)" }}>
          {c.tag}
        </div>
      ...
      <div className="p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs mt-1 font-inter" style={{ color: c.accent }}>{c.issuer}</p>
            <p className="text-muted text-[11px] font-inter mt-0.5">{c.date}</p>
            <p className="text-muted text-xs font-inter mt-2 leading-relaxed line-clamp-2">{c.desc}</p>
          </div>
        </div>
      </div>
    </motion.div>
          ...
          >
            ...
            >
              <div className="relative w-full aspect-[4/3]">
                <Image src={selected.image} alt={selected.title} fill className="object-contain p-4" />
              </div>
              <div className="p-6 border-t border-white/5">
                <p className="font-grotesk font-bold text-offwhite">{selected.title}</p>
                <p className="text-xs font-inter mt-1" style={{ color: selected.accent }}>{selected.issuer} · {selected.date}</p>
                <p className="text-muted text-xs font-inter mt-2 leading-relaxed">{selected.desc}</p>
              </div>
